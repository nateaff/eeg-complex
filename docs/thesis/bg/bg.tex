...
\section{Epsilon-complexity} 
  ...
  % functions, stochastic processes, and observed time series 
  % and in 
...
% begin edit -------------------------------------------------------
  ...
  The relationship characterizes the 
...
\section{Time series}
  ...
  % The theory 
  % of $\epsilon-$complexity as developed in \cite{darkhovsky2013} for continuous and possibly vector-valued functions 
  % denoted $x(t)$. 
  In practice we observe time series as a set of discrete 
...
% Some stochastic processes can be specified entirely by their autocorrelation functions 
...
%   to analyze a neonate EEG signal.
  ...
  % approximation methods could include, and a larger set 
  ...
  % of approximation. 
  % A couple of choices need to be made to for a practical  
...
Basis or B-splines are piecewise polynomials. 
